...
workingDir = "D:\\Test_WGCNA\\Left_T_N\\WCGNA"######working dir should be changed######
setwd(workingDir)
getwd()
...
fpkm = read.table("111Normal_left-vs-Tumor_Left.limma_Dif.matrixtxt.txt",header=T,comment.char = "",check.names=F)#########file name can be changed#####数据文件名，根据实际修改，如果工作路径不是实际数据路径，需要添加正确的数据路径
...
##filter
meanFPKM=0.5  ####the threshold can be changed
n=nrow(datExpr0)
...
head(filtered_fpkm)
write.table(filtered_fpkm, file="Xena.CRC_filter.xls",row.names=F, col.names=T,quote=FALSE,sep="\t")
...
############### input clinical trait data###############
#Loading clinical trait data
traitData=read.table("Clinical.WGCNA.MRE11.txt",sep="\t",row.names=1,header=T,check.names=F)########trait file name can be changed######性状数据文件名，根据实际修改，如果工作路径不是实际性状数据路径，需要添加正确的数据路径
dim(traitData)
names(traitData)
...
#Evaluate the softPower if it satisify scale free。
ADJ1 =abs(cor(datExpr0,use="p"))^3 
k=as.vector(apply(ADJ1,2,sum,na.rm=T))
sizeGrWindow(10,5)
pdf(file="3-1_Scale test.pdf",width=9,height=5)
par(mfrow=c(1,2))
hist(k)
...
plotDiss = selectTOM^3  ##modify the plotDiss as exact softpower
diag(plotDiss) = NA
...
## Add the interesting parameters to module eigengenes
##Recalculate module eigegenes
MEList = moduleEigengenes(datExpr0, colors = dynamicColors)
MEs = MEList$eigengenes
MRE11A.L_H = as.data.frame(datTraits$MRE11A.L_H);
names(MRE11A.L_H) = "MRE11A.L_H" 
MET = orderMEs(cbind(MEs,MRE11A.L_H)) 
sizeGrWindow(5,7.5);
pdf(file="14_1_Eigengene dendrogram_2.pdf",width=6, height=6)
par(cex = 0.9)
plotEigengeneNetworks(MET, "", marDendro = c(0,8,1,5), marHeatmap = c(8,8,1,2), cex.lab = 0.8, xLabelsAngle = 90)
dev.off()

##several parameters to existing module eigengenes
MRE11A.L_H = as.data.frame(datTraits$MRE11A.L_H);
MRE11A = as.data.frame(datTraits$MRE11A);
names(MRE11A.L_H) = "MRE11A.L_H"
names(MRE11A) = "MRE11A" #
MET = orderMEs(cbind(MEs,MRE11A.L_H,MRE11A)) 
sizeGrWindow(5,7.5);
pdf(file="14_2_Eigengene dendrogram_2.pdf",width=6, height=6)
par(cex = 0.9)
plotEigengeneNetworks(MET, "", marDendro = c(0,8,1,5), marHeatmap = c(8,8,1,2), cex.lab = 0.8, xLabelsAngle = 90)
dev.off()
